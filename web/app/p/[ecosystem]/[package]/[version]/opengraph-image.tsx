...
import { scanPackage, PackageNotFoundError, ECOSYSTEMS, type Ecosystem } from "@/lib/scan";
...
// See page.tsx in this folder for why params are decoded explicitly here.
...
export default async function OpengraphImage({
  ...
  params: { ecosystem: string; package: string; version: string };
}) {
  const packageName = decodeParam(params.package);
  const version = decodeParam(params.version);
  const decodedEcosystem = decodeParam(params.ecosystem);
  const ecosystem: Ecosystem = isEcosystem(decodedEcosystem) ? decodedEcosystem : "npm";
      ...
      (
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", fontSize: 20, color: "#64748b", fontWeight: 600 }}>
              {result.ecosystem}
            </div>
            <div style={{ display: "flex", fontSize: 44, color: "#1e293b", fontWeight: 700, marginTop: 4 }}>
              {result.package}
          ...
          <div style={{ display: "flex", fontSize: 22, color: "#64748b" }}>
            packagesafe.dev/p/{result.ecosystem}/{result.package}/{result.resolved_version}
          </div>
        </div>
      ),
      { ...size }
      ...
      (
        ...
        >
          ...
          <div style={{ display: "flex", fontSize: 28, color: "#64748b" }}>
            {notFound
              ? `'${packageName}@${version}' was not found in ${ecosystem}`
              : "Unable to scan this package"}
          </div>
        </div>
      ),
      { ...size }
